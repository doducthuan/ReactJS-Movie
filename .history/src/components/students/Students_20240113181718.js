...
const Students = () => {
    ...
    const addStudent = async () => {
        try {
            const update = await axios.post(GETALL,initForm);
            return update.data;
        }
        catch (error) {
            console.log(error);
    ...
    const updateStudent = async () => {
        try {
            const update = await axios.put(`http://localhost:3003/students/${inputValue.id}`, initForm);
            return update.data;
        }
        catch (error) {
            console.log(error);
    ...
    const validateData = (finalForm) => {
        console.log(englishRef.current.value);
        const item = {id:idRef.current.value, name: nameRef.current.value, math: mathRef.current.value, literature: literatureRef.current.value, english:englishRef.current.value};
        console.log(item);
        setFinalForm(item)
                                ...
                                <div className="border-0 rounded-lg shadow-lg flex relative flex-col w-[550px] h-[670px] bg-white outline-none focus:outline-none">
                                    <div className='w-[450px] h-[600px] m-[50px]'>
                                        ...
                                        <div className="flex gap-3 mt-8 pl-[155px]">
                                            ...
                                            <button className="w-[100px] h-[40px] border-solid border-2 bg-primary text-white" onClick={() =>validateData(finalForm)}>Cập nhật</button>
                                        </div>
                                    </div>
                                </div>
