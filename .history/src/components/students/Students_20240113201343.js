...
const Students = () => {
    ...
    const getAllData = async () => {
        try {
            const update = await axios.get(GETALL);
            return update.data;
        }
        catch (error) {
            console.log(error);
    ...
    useEffect(() => {
        async function handData() {
            ...
            dataSearch.length === 0 ? setData(full_data) : setData(dataSearch);
        }
        handData();
    }, [page, reGen, dataSearch]);
    useEffect(() => {
        async function totalData() {
            ...
            console.log(page_tt + 1);           
        }
        totalData();
    }, [reGen, page, dataSearch]);
    const editItem = (id) => {
        if(id > 0){
            ...
            async function get_all_data(){
                ...
                if(allData.length > 0){
                    ...
                    allData.forEach((item, index) =>{
                        if(item.id === floatValue || item.math === floatValue || item.literature === floatValue || item.english === floatValue || item.name.includes(e.target.value)){
                ...
                <div className={!showModal ? "border-data" : "border-data opacity-50"}>
                    ...
                    <div className="table-data">
                        <div className="search-add mt-9 flex">
                            <input className="search-all text-lg" placeholder='Tìm kiếm ...' onKeyDown={searAll}/>
