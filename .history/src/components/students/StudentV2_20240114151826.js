    ...
    useEffect(() => {
        async function handData() {
            if(searchAll === ""){
                ...
                const data_search_all = await searchAllData(valueSearchAll.current);
                if(data_search_all.length > 0){
                    setCount(data_search_all.length);
                    let startIndex = (page - 1) * 4;
                    let endIndex = (page - 1) * 4 + 4;
    ...
    const searchAll = (e) => {
        if(e.keyCode === 13){
            async function search_all(){
                ...
                valueSearchAll.current = e.target.value;
                setPage(1);
            }
            search_all();
        }
