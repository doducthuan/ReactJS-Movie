...
const StudentV2 = () => {
    ...
    const searchAllData = async (value, sort) => {
        try {
            const update = await axios.get(`http://localhost:3003/students?_sort=${sort}&q=${value}`);
            return update.data;
        }
        catch (error) {
            console.log(error);
    ...
    const getPagingData = async (page, sort) => {
        try {
            const response = await axios.get(`http://localhost:3003/students?_sort=${sort}&_page=${page}&_limit=4`);
            return response.data;
        }
        catch (error) {
            console.log(error);
    ...
    useEffect(() => {
        async function handData() {
            if(valueSearchAll === ""){
                const full_data = await getPagingData(page, sort);
                setData(full_data);
                let countData = await getTotalData();
                setCount(countData);
            }
            else{
                const data_search_all = await searchAllData(valueSearchAll, sort);
        ...
        handData();
    }, [page, reGen, count, valueSearchAll, sort]);
    useEffect(() => {
                        ...
                        <div>
                            <table>
                                <thead>
                                    <tr>
                                        <th>STT</th>
                                        <th className='cursor-pointer' onClick={() => setSort("id")}>MSSV</th>
                                        <th className="cursor-pointer" onClick={() => setSort("name")}>Họ và tên</th>
                                        <th className="cursor-pointer" onClick={() => setSort("math")}>Toán</th>
                                        <th className="cursor-pointer" onClick={() => setSort("literature")}>Văn</th>
                                        <th className="cursor-pointer" onClick={() => setSort("english")}>Anh</th>
                                        <th>Thao tác</th>
                                    </tr>
                                </thead>
                                <tbody>
